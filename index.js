...
client.on('ready', () => {
    console.log(`Unusual Den Member Count: ${client.guilds.cache.get('901004399252496424').memberCount}`)

    const guild = client.guilds.cache.get('901004399252496424').memberCount
...
client.on('messageCreate', async message => {
    ...
    const command = cmd.toLowerCase();
        ...
        if(command === 'role') {
            ...
            .addComponents([
                new discord.MessageButton().setLabel('Announcement Ping').setStyle("SECONDARY").setCustomId('announcement-ping').setEmoji('🔔'),
                new discord.MessageButton().setLabel('Server Update Ping').setStyle("SECONDARY").setCustomId('server-ping').setEmoji('🌠')
